...
function dragSort(container, handleClass, scrollElement = null) {
	...
	function userPressed(event) {
		if (singleClick) {
			var layer = layers.find(
				(element) => element.layerElement == event.target
			);
	...
	function returnRenderLoop() {
		curScale = lerp(curScale, neutralScale, 0.25);
		curPosY = lerp(curPosY, posDestY, 0.25);
		curPosX = lerp(curPosX, posDestX, 0.25);
		...
		draggingElement.it.style["transform"] =
			...
			"px," +
			(curPosY - posDestY) +
			"px) scale(var(--s), var(--s))";
		retRaf = requestAnimationFrame(returnRenderLoop);
		if (
			Math.abs(neutralScale - curScale) < 0.01 &&
			Math.abs(curPosY - posDestY) < 0.01 &&
			Math.abs(curPosX - posDestX) < 0.01
		) {
			draggingElement.it.style.setProperty("--s", neutralScale);
			draggingElement.it.style["transform"] =
				"translate(0px, 0px) scale(var(--s), var(--s))";
			endReturnRenderLoop();
		}
	}
	...
	function mainRenderLoop() {
		curScale = lerp(curScale, scaleDest, 0.2);
		draggingElement.it.style.setProperty(
			"--s",
			Math.round(curScale * 100) / 100
		);
		translateFloatingItem(lastClientX, lastClientY);
		updateScroll(lastClientX, lastClientY);

		draggingElement.centerOffsetX = lerp(
			draggingElement.centerOffsetX,
			draggingElement.targetCenterOffsetX,
			0.6
		);
		draggingElement.centerOffsetY = lerp(
			draggingElement.centerOffsetY,
			draggingElement.targetCenterOffsetY,
			0.6
		);
	...
	function initFloatingItem(target, clientX, clientY) {
		...
		draggingElement.targetCenterOffsetX =
			draggingElement.bbox.width * 0.5 - (clientX - draggingElement.bbox.left);
		draggingElement.centerOffsetY = 0;
		draggingElement.targetCenterOffsetY =
			draggingElement.bbox.height * 0.5 - (clientY - draggingElement.bbox.top);
		...
		var nodes = Array.prototype.slice.call(
			draggingElement.it.parentElement.children
		);
		...
		container.style["padding-bottom"] =
			...
			(draggingElement.bbox.height +
				(oIndex == draggingElement.it.parentElement.children.length - 1
					? 0
					: 8)) +
			"px";
	}
	...
	function translateFloatingItem(clientX, clientY) {
		...
		elementPositionY =
			clientY + draggingElement.offsetY - draggingElement.centerOffsetY - 50;
		elementPositionX =
			clientX - draggingElement.offsetX - draggingElement.centerOffsetX;
	...
	function restoreFloatingItem(lastClientY) {
		...
		curPosX = activePosX;
		posDestY = 69 * calculateListIndex(lastClientY + getScroll()) + 60;
		posDestX = 0;
		draggingElement.it.style["position"] = draggingElement.position;
		draggingElement.it.style["z-index"] = draggingElement.zIndex;
		clearGap();

		moveItemTo(draggingElement, calculateListIndex(lastClientY + getScroll()));
	...
	function calculateListIndex(yInListSpace) {
		...
		if (
			yInListSpace >= intersectionStructure[intersectionStructure.length - 1]
		) {
	...
	function drawGap(position, size) {
		...
		for (var i = firstVisible; i < lastVisible + 1; i++) {
			...
			if (i === draggingElement.index) {
				...
				it.style.transform =
					"translateY(" + (size + 8) + "px) scale(var(--s), var(--s))";
			}
		}
	}
	...
	function timedStart(eventTarget, startX, startY, delay, continueCallback) {
		console.log(delay);
